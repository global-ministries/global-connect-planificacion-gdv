/**
 * W06 — DT-039 — POST /api/pastoral/one-on-one/[id]/schedule
 *
 * Reschedules a pastoral 1:1 (updates scheduledAt + applies state transition).
 * Uses optimistic locking via expectedVersion → 409 stale.
 *
 * Transitions:
 * - pending_participant → scheduled (schedule action)
 * - scheduled → scheduled (re-schedule, stays in scheduled)
 *
 * Access: mentor autor or pastoral.read.all.
 */
...
function parseScheduleBody(body: unknown): {
  ...
  if (!scheduledAt) return { error: 'scheduledAt es requerido' }
  ...
  return { scheduledAt, expectedVersion }
}
...
export async function POST(req: NextRequest, context: RouteContext) {
  try {
    ...
    // Only mentor autor can schedule
    ...
    console.error('[pastoral/one-on-one/[id]/schedule POST] error:', error)
